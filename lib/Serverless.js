...
class Serverless {
  ...
  command(argv) {
    ...
    // If SERVERLESS_COMPLETION env var is set, refresh the completion cache data.
    if(Boolean(process.env['SERVERLESS_COMPLETION'])) {
      _this.refreshCompletionCache(_this.commands, _this._version, _this.config.projectPath);
    }
  ...
  /**
   * Refreshes the completion cache used for bash tab completions.
   */
  refreshCompletionCache(serverlessCommands, serverlessVersion, projectPath) {
    // Completion Dependencies
    const lib = require('./utils/completion');
    ...
    completionData = lib.parseCommandsObject(serverlessCommands);
    ...
    this.utils.writeFileSync(completionCacheFilename, completionData);
  }
